        ...
        try:
            ...
            async def main():
                ...
                async with AsyncSession(headers=headers) as http_client:
                    ...
                    response = await http_client.post(url="https://gifts2.tonnel.network/api/balance/info", json=jsondata, proxy=ROTATED_PROXY, timeout=10)
                    if response.ok:
                    ...
                    for giveaway_code in giv_ids_ozim:
                        ...
                        response = await http_client.post(url="https://gifts2.tonnel.network/api/giveaway/info", headers=headers, proxy=ROTATED_PROXY, json=jsondata, timeout=20)
